...
async function logActivity(req, action, productId, metadata = {}) {
  ...
  } catch (e) { console.error("Review activity:", e.message); }
}
...
exports.create = async (req, res) => {
  try {
    ...
    if (comment.length < 3) return res.status(400).json({ success: false, message: "Review must contain at least 3 characters." });
    if (comment.length > 2000) return res.status(400).json({ success: false, message: "Review is too long." });
    ...
    await logActivity(req, "REVIEW_CREATE", productId, { reviewId: review.id, rating });
    ...
    return res.status(201).json({ success: true, message: "Review published successfully.", review: payload });
  } catch (error) {
    console.error("CREATE REVIEW:", error);
    if (error.name === "SequelizeUniqueConstraintError") return res.status(409).json({ success: false, message: "You have already reviewed this product." });
...
exports.testimonials = async (req, res) => {
  try {
    const reviews = await Review.findAll({
      ...
      include: [
        { model: User, as: "user", attributes: ["id", "name", "avatar", "city"] },
        { model: Product, as: "product", attributes: ["id", "name", "image"] },
      ],
      order: [["createdAt", "DESC"]],
      limit: 20,
    });
    const fallback = reviews.length ? reviews : await Review.findAll({
    ...
    return res.json({
      ...
      testimonials: fallback.map(r => ({
        ...
        product: r.product ? { id: r.product.id, name: r.product.name, image: r.product.image } : null,
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    const io = req.app.get("io");
    if (io) {
      io.to(`product:${review.product_id}`).emit("product:review:updated", payload);
      io.emit("review:updated", payload);
    }
    return res.json({ success: true, message: `Review ${status}.`, review: payload });
  } catch (error) {
    console.error("UPDATE REVIEW:", error);
    return res.status(500).json({ success: false, message: "Unable to update review status." });
  }
};
...
exports.feature = async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: review.is_featured ? "Review featured." : "Review removed from testimonials.", is_featured: review.is_featured });
  } catch (error) {
