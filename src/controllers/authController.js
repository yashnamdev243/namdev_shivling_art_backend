...
exports.register = async (req, res) => {
  try {
    ...
    if (existing) return res.status(409).json({ success: false, message: "An account with this email already exists. Please login." });
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user || user.role !== "user") return res.status(401).json({ success: false, message: "Invalid email or password." });
    if (user.status !== "active") return res.status(403).json({ success: false, message: "Your account is inactive. Please contact us." });
    ...
    return res.json({ success: true, message: "Login successful.", token: generateToken(user), user: publicUser(user) });
  } catch (error) {
...
exports.me = async (req, res) => {
  try {
    ...
    if (!user || user.role !== "user") return res.status(404).json({ success: false, message: "User not found." });
