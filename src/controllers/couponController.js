...
//       code: "NOT_FOUND",
//       message:
//         "We couldn't find a coupon with that code. Please check and try again.",
//     };
//   }
...
//       valid: false,
//       code: "NOT_STARTED",
//       message: `This coupon isn't live yet. It starts on ${formatDate(coupon.starts_at)}.`,
//     };
//   }
...
//       valid: false,
//       code: "EXPIRED",
//       message: `This coupon expired on ${formatDate(coupon.expires_at)}.`,
//     };
//   }
...
//       valid: false,
//       code: "INVALID_PRICE",
//       message: "This product can't be used with a coupon right now.",
//     };
//   }
...
//         valid: false,
//         code: "NO_PRODUCTS",
//         message: "This coupon isn't linked to any products yet.",
//       };
//     }
...
//         message: "This coupon isn't valid for this product.",
//       };
//     }
//   }
...
//     return res
//       .status(500)
//       .json({ success: false, message: "Unable to apply coupon." });
//   }
// };
...
//     return res
//       .status(500)
//       .json({ success: false, message: "Unable to load your coupon history." });
//   }
// };
...
//     return res
//       .status(500)
//       .json({ success: false, message: "Unable to load coupon redemptions." });
//   }
// };
...
//     return res
//       .status(500)
//       .json({ success: false, message: "Unable to load active coupons." });
//   }
// };
...
//     const perUserLimit = Number(per_user_limit || 1);
...
//       ? [...new Set(product_ids.map(Number).filter(Boolean))]
//       : [];

//     const appliesToAll = applies_to_all !== false && applies_to_all !== "false";
...
//     if (productIds.length > 0 && CouponProduct) {
...
//       coupon.title = body.title || null;
//     }

//     if (body.description !== undefined) {
//       coupon.description = body.description || null;
//     }
...
//       if (coupon.discount_type === "percentage" && value > 100) {
//         return res.status(400).json({
//           success: false,
//           message: "Percentage discount cannot exceed 100.",
//         });
...
//       coupon.min_order_amount = Number(body.min_order_amount || 0);
//     }
...
//       coupon.per_user_limit = Number(body.per_user_limit || 1);
//     }

//     if (body.starts_at !== undefined) {
//       coupon.starts_at = body.starts_at || null;
//     }

//     if (body.expires_at !== undefined) {
//       coupon.expires_at = body.expires_at || null;
//     }

//     if (body.is_active !== undefined) {
//       coupon.is_active = body.is_active !== false && body.is_active !== "false";
//     }

//     if (body.festival_name !== undefined) {
//       coupon.festival_name = body.festival_name || null;
//     }

//     if (body.applies_to_all !== undefined) {
//       coupon.applies_to_all =
//         body.applies_to_all !== false && body.applies_to_all !== "false";
//     }
...
//     if (Array.isArray(body.product_ids) && CouponProduct) {
//       const productIds = [
//         ...new Set(body.product_ids.map(Number).filter(Boolean)),
//       ];
...
try {
  ...
  console.warn("CouponProduct model not found. Product-specific coupon support will be limited.");
}
...
async function getCouponProductIds(couponId, t) {
...
/* =========================================================
   VALIDATE COUPON (transaction-aware — used by /apply)
   `coupon` is passed in already fetched + row-locked so this
   function never re-queries it, keeping everything consistent
   within the same transaction.
========================================================= */
async function validateCouponForProductLocked(coupon, product, userId, t) {
  if (!coupon) {
  ...
  if (!coupon.applies_to_all) {
    const allowedProductIds = await getCouponProductIds(coupon.id, t);
    if (allowedProductIds.length === 0) {
  ...
  if (coupon.usage_limit) {
    const totalUsed = await CouponRedemption.count({ where: { coupon_id: coupon.id }, transaction: t });
    if (totalUsed >= coupon.usage_limit) {
  ...
  if (userId) {
    const usedByUser = await CouponRedemption.count({ where: { coupon_id: coupon.id, user_id: userId }, transaction: t });
    const limit = coupon.per_user_limit || 1;
    if (usedByUser >= limit) {
      return {
        valid: false,
...
/* =========================================================
   CUSTOMER — POST /api/coupons/apply
   Wrapped in a transaction with a row lock on the coupon so
   two simultaneous requests (double-click, script) can never
   both pass validation and double-redeem the same coupon.
========================================================= */
exports.apply = async (req, res) => {
  ...
  try {
    ...
    const result = await validateCouponForProductLocked(coupon, product, userId, t);
...
/* Customer: active coupons — cached for 60s since this is read very often */
exports.active = async (req, res) => {
  try {
    const cached = cache.get("active-coupons");
    ...
    cache.set("active-coupons", coupons, 60);
    return res.json({ success: true, coupons });
  } catch (error) {
    console.error("ACTIVE COUPONS ERROR:", error);
...
exports.getOne = async (req, res) => {
  try {
    ...
    console.error("GET COUPON ERROR:", error);
    return res.status(500).json({ success: false, message: "Unable to load coupon." });
  }
};
...
exports.create = async (req, res) => {
  try {
    const {
      ...
      starts_at, expires_at, is_active, festival_name, applies_to_all, product_ids,
    } = req.body;
    ...
    const minAmount = Number(min_order_amount || 0);
    if (!Number.isFinite(minAmount) || minAmount < 0) {
      return res.status(400).json({ success: false, message: "Invalid minimum order amount." });
    }

    let maxDiscount = null;
    if (max_discount !== "" && max_discount !== null && max_discount !== undefined) {
      maxDiscount = Number(max_discount);
      if (!Number.isFinite(maxDiscount) || maxDiscount < 0) {
        return res.status(400).json({ success: false, message: "Invalid maximum discount." });
      }
    }

    let usageLimit = null;
    if (usage_limit !== "" && usage_limit !== null && usage_limit !== undefined) {
      usageLimit = Number(usage_limit);
      if (!Number.isInteger(usageLimit) || usageLimit <= 0) {
    ...
    const perUserLimit = Number(per_user_limit || 1);
    if (!Number.isInteger(perUserLimit) || perUserLimit <= 0) {
      return res.status(400).json({ success: false, message: "Per-user limit must be a positive number." });
    }
    ...
    if (productIds.length > 0) {
      const products = await Product.findAll({ where: { id: productIds }, attributes: ["id"] });
      if (products.length !== productIds.length) {
        return res.status(400).json({ success: false, message: "One or more selected products do not exist." });
      }
    }
    ...
    if (productIds.length > 0 && CouponProduct) {
      await CouponProduct.bulkCreate(productIds.map((product_id) => ({ coupon_id: coupon.id, product_id })));
    }

    cache.del("active-coupons");
...
exports.update = async (req, res) => {
  try {
    ...
    if (body.code !== undefined) {
      ...
      const duplicate = await Coupon.findOne({ where: { code: newCode, id: { [Op.ne]: coupon.id } } });
      if (duplicate) return res.status(409).json({ success: false, message: "Coupon code already exists." });

      coupon.code = newCode;
    }

    if (body.title !== undefined) coupon.title = body.title || null;
    if (body.description !== undefined) coupon.description = body.description || null;
    ...
    if (body.max_discount !== undefined) {
      coupon.max_discount = body.max_discount === "" || body.max_discount === null ? null : Number(body.max_discount);
    }

    if (body.usage_limit !== undefined) {
      coupon.usage_limit = body.usage_limit === "" || body.usage_limit === null ? null : Number(body.usage_limit);
    }

    if (body.per_user_limit !== undefined) coupon.per_user_limit = Number(body.per_user_limit || 1);
    if (body.starts_at !== undefined) coupon.starts_at = body.starts_at || null;
    if (body.expires_at !== undefined) coupon.expires_at = body.expires_at || null;
    if (body.is_active !== undefined) coupon.is_active = body.is_active !== false && body.is_active !== "false";
    if (body.festival_name !== undefined) coupon.festival_name = body.festival_name || null;
    if (body.applies_to_all !== undefined) {
      coupon.applies_to_all = body.applies_to_all !== false && body.applies_to_all !== "false";
    }

    await coupon.save();

    if (Array.isArray(body.product_ids) && CouponProduct) {
      const productIds = [...new Set(body.product_ids.map(Number).filter(Boolean))];

      if (!coupon.applies_to_all && productIds.length === 0) {
        return res.status(400).json({ success: false, message: "Select at least one product." });
      }

      await CouponProduct.destroy({ where: { coupon_id: coupon.id } });
